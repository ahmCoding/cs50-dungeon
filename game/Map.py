...
class Map:
    ...
    def _create_map(self):
        ...
        tmp_map = [
            [self._map_chars["wall"] for w in range(self._width)]
    ...
    @classmethod
    def get_map_obj_from_grid(cls, grid: list[list[str]]) -> Self:
        tmp_obj = cls()
    ...
    def get_game_map(self) -> list[list[str]]:
        return [
            [self._map[h][w] for w in range(self._width)] for h in range(self._height)
        ]
